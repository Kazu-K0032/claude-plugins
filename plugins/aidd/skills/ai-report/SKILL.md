...
- `SUMMARY` / `PROMPTS` ブロック: typed プロンプト一覧から良い点・改善点を執筆する。総評で数値に触れるときは Step 2 の出力をそのまま引用し、推測で作らない。「セッションテーマ一覧」に意味的に似た作業テーマが複数あれば、そのテーマの永続カスタムエージェント化の検討を SUMMARY に 1 文添える（無ければ言及しない）
